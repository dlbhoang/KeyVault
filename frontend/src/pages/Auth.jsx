      ...
      >
        ...
        <div style={{ padding:'32px 34px' }}>
          <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:28 }}>
            ...
            {badge && (
              <span style={{ marginLeft:'auto', fontSize:10, fontWeight:700, background:'var(--indigo-l)', color:'var(--indigo-d)', padding:'3px 9px', borderRadius:20, letterSpacing:'.06em' }}>
                {badge}
              </span>
            )}
          </div>
...
export function UserRegister() {
  ...
  const submit = async () => {
    ...
    if (await register(name, email, pass)) navigate('/portal', { replace: true })
  }
    ...
    <AuthCard title="Quên mật khẩu?" subtitle="Nhập email để nhận link đặt lại mật khẩu">
      {result ? (
        <motion.div initial={{ opacity:0, scale:.96 }} animate={{ opacity:1, scale:1 }}>
          ...
          <Btn variant="ghost" onClick={() => navigate('/login')} full>
...
export function ResetPassword() {
  const navigate = useNavigate()
  const { token } = useParams()                          // lấy từ /reset-password/:token
